...
def post_add(request):
    if request.method == 'POST':
        ...
        icerikGelen = request.POST.get('icerikGelen')
...
def ai_cek(request):
    if request.method == 'POST':
        ...
        if kontrols.exists():
            ...
            setattr(random_kontrol, field, 'Kullanıldı')
            random_kontrol.save()
...
def ai_makale_cek(request):
    if request.method == 'POST':
        ...
        if kontrols.exists():
            ...
            random_kontrol.save()
            Sonucu = f"{random_kontrol.pk}|={random_kontrol.icerikGelen}|={random_kontrol.AltBasliklar}"
            return HttpResponse(Sonucu)
        else:
